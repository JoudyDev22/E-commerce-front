...
export default function TableShow(props){
    ...
    const showdata=showWichData.map((item,index)=>
        ...
        <tr key={index}>
            ...
            (
            <td key={index2}>{
                item2.key==='image'?(<img src={"https://ecommerce-back-production-1231.up.railway.app"+ item[item2.key]} alt="img" style={{width:'40px'}}></img>):
                ...
                {item[item2.key].map((img,index)=>(
                        <img src={"https://ecommerce-back-production-1231.up.railway.app"+img.image} alt="img" key={index} style={{width:'40px'}}></img>
                    ))}</div>)
